...
当前版本：`0.0.1-beta.15`。这是以移动端阅读为重点的早期 Beta，已具备内容后台、版本发布、读者纠错、分层阅读、专题路线、收藏、继续阅读、分享卡片和公开页面服务端渲染；“心脏与血液循环”和“呼吸与肺泡气体交换”两个互动专题已经可用。
...
- **动手理解身体过程**：在心脏页逐步观察血液循环，在肺页跟随空气到达肺泡并理解气体交换；均支持器官结构点选、静态全文和减少动态效果，不设置答题或评分负担。
- **查找知识**：按关键词搜索，使用分类筛选与分页浏览专题。
- **追溯来源**：从正文引用定位到参考资料，查看发布机构和来源链接。
- **收藏与分享**：在当前浏览器保存收藏；指标和器官可生成带二维码、内容核对状态与科普提示的 PNG 卡片，下载或通过支持的系统分享入口转发，也可复制阅读链接。
...
- **公开页面**：完整部署时直接返回正文 HTML、独立标题与分享摘要；未知或撤回内容返回 404，待核对演示不参与搜索收录。
- **按需阅读**：文章目录直达、大字号记忆，以及可手动开启、清空和关闭的本地阅读记录；默认不记录健康兴趣。
- **先看重点**：指标页提供原文速览和可展开的术语问答，保留完整限制条件与来源，不自动生成医学结论。
- **表达阅读感受**：可选择「有帮助」「仍没看懂」「没找到想了解的」，确认后发送给编辑，凭码查询或删除；不要求填写个人健康信息。

当前包含 3 个指标专题和 5 个器官专题。收藏保存在浏览器本地，不支持跨设备同步。后台采用 Payload CMS + PostgreSQL，支持来源管理、知识编辑、来源整理发布、版本记录、回滚和撤回；未提供读者账号、App 或小程序客户端。

正式内容 API 与待核对演示 API 相互独立。基础解剖、生理和术语在逐项核对具体来源、许可、中文表达与示意边界后即可发布，并明确显示“依据来源整理”，不要求独立专业审校。疾病、数值标准、特殊人群和诊疗内容不在当前产品范围。旧版专业审校数据结构仅作为兼容能力保留，不是内容生产前置条件；现有内容也不会因规则调整自动进入正式发布集合。后台使用及部署见 [内容平台说明](docs/content-platform.md)。

## 内容说明

专题包含内容版本、更新时间、来源整理状态和参考资料，来源包括 WHO、NIDDK、NHLBI、NLM / MedlinePlus。“依据来源整理”不表示来源机构为知愈背书；各篇状态以页面当前发布版本为准。
...
默认使用项目内的静态内容，无需配置环境变量或启动后端。启用本项目的内容平台后，可参考 `.env.example` 设置 `VITE_CONTENT_API_BASE_URL=/api/demo` 使用待核对演示集合，或设置为 `/api/v1` 使用正式发布集合。后端启动、账号和迁移说明见 [内容平台说明](docs/content-platform.md)。
...
后台提供独立数据库中的权限、来源整理发布、版本切换、回滚与撤回测试，不会向正式数据库写入测试身份或数据。
...
- [分享卡片与安全边界](docs/share-cards.md)
- [分层阅读与阅读反馈](docs/layered-reading.md)
- [可视化互动首期方案](docs/visual-learning.md)
